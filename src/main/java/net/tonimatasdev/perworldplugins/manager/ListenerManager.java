...
public class ListenerManager {
    public static void convert(Plugin plugin) {
        ...
        for (RegisteredListener registeredListener : HandlerList.getRegisteredListeners(plugin)) {
            if (registeredListener instanceof PerWorldRegisteredListener) continue; // If RegisteredListener is already converted it passes.

            // Get all HandlerList for get RegisteredListener HandlerList.
            for (HandlerList handlerList : HandlerList.getHandlerLists()) {
                if (!Arrays.asList(handlerList.getRegisteredListeners()).contains(registeredListener)) continue; // See if the RegisteredListener is in this HandlerList or not.
